<?php
		...
		class WKPDF {
                ...
                public function __construct(){
					$this->cmd='/usr/local/bin/wkhtmltopdf';
                ...
                public function render(){
                        $web='/var/www/vhosts/meritnation.com/subdomains/testpro/httpdocs/app/webroot/tmp/'.basename($this->tmp);
...
/*                        if(strpos(strtolower($this->pdf['stderr']),'error')!==false)throw new Exception('WKPDF system error: <pre>'.$this->pdf['stderr'].'</pre>');
                        ...
                        if(isset ($this->pdf['stdout']) && $this->pdf['stdout']=='')throw new Exception('WKPDF didn\'t return any data. <pre>'.$this->pdf['stderr'].'</pre>');

                        if(isset($this->pdf['return']) && ((int)$this->pdf['return'])>1)throw new Exception('WKPDF shell error, return code '.(int)$this->pdf['return'].'.');*/
